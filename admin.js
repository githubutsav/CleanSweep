    ...
    if (window.supabaseClient) {
        try {
            const { data, error } = await window.supabaseClient
                ...
                .limit(200);
            if (!error && data) {
                // console.log('[ADMIN] Supabase returned rows:', data.length, data);
                reports = data.map(r => ({
                    lat: r.latitude,
                    lon: r.longitude,
...
function renderMap() {
    ...
    if (!mapDiv) {
        console.error('[ADMIN] Map container #admin-map not found in DOM!');
        return;
    }
    if (adminMap) {
        adminMap.remove();
    ...
    const active = reports.filter(r => !['done','archived'].includes(r.status));
    // console.log('[ADMIN] Rendering map with active reports:', active.length, active);
    active.forEach(addReportMarker);
}
...
function renderList() {
    ...
    if (!list || !historyList) {
        console.error('[ADMIN] List containers not found in DOM!');
        return;
    }
    list.innerHTML = '';
    ...
    // console.log('[ADMIN] Rendering list. Active:', actives.length, actives, 'History:', history.length, history);
    ...
    container.addEventListener('click', async (e) => {
        ...
        try {
            if (window.supabaseClient && id) {
                ...
                // Always reload from Supabase after status update
                await loadReports();
                renderMap();
                renderList();
            }
